...
form.addEventListener("submit", function (e) {
  ...
  //submit의 기본 기능은 페이지를 재로딩하는것이다. 그걸 막아준다.
  e.preventDefault();

  //value 값에 숫자 amount값을 넣는다.
...
// isNaN  (Is Not a Number)
  ...
  // case 1. value값이 숫자값이 아니라면? 
  // value가 숫자형이 아니거나(or) 0보다 작거나(or) 9보다 크거나 하면
  if (isNaN(value) || value < 0 || value > 9) {
    //이상한 값이 들어오면 랜덤값 출력한다. 
    result.innerHTML = `<p class="result">${text[random]}</p>`;
  } else {
    //아니면 
    //slice 메소드는 시작 인덱스부터 종료 인덱스 전까지의 
    //요소를 선택하여 새로운 배열을 만듭니다. 
    //시작 인덱스는 포함되고, 종료 인덱스는 제외됩니다.
